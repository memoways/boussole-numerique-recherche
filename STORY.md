...
| Révocation visible dans l’administration | Un lien d’invitation doit pouvoir être désactivé par l’équipe de projet sans manipulation SQL ni retrait des données déjà collectées | La console liste les invitations actives ; l’action Révoquer appelle l’endpoint déjà protégé et rend immédiatement le jeton inutilisable |
| Pilote dissocié du workflow e-mail | La collecte et ses contrôles ne doivent pas être bloqués par un service transactionnel encore en configuration | Le parcours partenaire, l’intérêt artiste, les exports, la régénération de l’outbox et la révocation peuvent être validés sans Dreamlit ; aucun e-mail ne doit être promis ou envoyé avant publication du workflow |
| Prompts de configuration séparés | Une instruction unique risque de mélanger les responsabilités de l’agent MCP, du workflow et de la validation humaine | Codex reçoit le cadre d’inspection et de proposition ; Dreamlit reçoit le périmètre des données, le modèle de message et les garde-fous. Publication et envoi de test restent deux validations explicites distinctes |
| Instrumentation GrainQL globale | La mesure doit couvrir les pages publiques sans créer une intégration disparate par écran | Le script GrainQL fourni est chargé une seule fois dans le document HTML racine ; les routes React et le questionnaire conservent leur comportement existant |
| Accueil comme tableau de cadrage | Un récit institutionnel centré rendait moins visible la nature de recherche et de co-conception du site | L’entrée combine un statut, trois métriques strictement descriptives, deux parcours et une frise de processus. La couleur devient un code de lecture : cadre indigo, flux cyan, test vert, action orange |
| Statuts de collecte visibles | Un environnement sans API ne doit pas ressembler à une page inachevée ou induire l’état de production en erreur | Les vues de transition utilisent une barre indigo, un libellé monospace, deux ou trois repères fonctionnels et une explication exacte de l’état de connexion |
...
| E-mails Dreamlit | Boîte d’envoi livrée, workflow non publié | Le transport PostgreSQL public répond en TLS ; résoudre la confiance du certificat Coolify côté Dreamlit avant de connecter puis publier le workflow |
| Manifestations d’intérêt | Frontend et API livrés | Contrôler une première soumission artiste de test et l’export dans `/admin` ; il ne s’agit pas encore d’un questionnaire artiste individuel |
