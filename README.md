...
Le projet est conçu pour être déployé et maintenu hors plateforme : le portail est une application statique servie par Nginx ; l’API partenaire est une application Express distincte ; PostgreSQL et l’API sont à activer dans Coolify avant le pilote. La zone Cloudflare utilise deux CNAME DNS only vers `lime.1024b.net`, tandis que le proxy Coolify conserve son challenge HTTP standard pour émettre les certificats si les ports 80 et 443 sont publics. Le DNS challenge Cloudflare est une option pour les wildcards, les serveurs sans port 80 ou un échec HTTP-01 confirmé. Dreamlit reste un service externe, à connecter seulement après validation du pilote de base.
