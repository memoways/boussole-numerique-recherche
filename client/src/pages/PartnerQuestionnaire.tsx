/**
 * Questionnaire partenaire — collecte par invitation, à progression guidée et à réponse vocale facultative.
 * La confirmation reste exacte durant le pilote, y compris tant que l’e-mail transactionnel n’est pas actif.
 */
...
function ServicePending() {
  return <section className="relative mx-auto max-w-2xl overflow-hidden border border-[#515792]/20 bg-[#f5f6fb] p-6 sm:p-8"><div className="absolute inset-y-0 left-0 w-1.5 bg-[#515792]" aria-hidden="true" /><div className="flex items-start gap-4"><span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#515792] text-white"><Sparkles className="h-6 w-6" /></span><div><p className="font-mono text-[11px] font-bold uppercase tracking-[0.16em] text-[#515792]">Statut · collecte en configuration</p><h1 className="mt-2 text-2xl font-extrabold tracking-tight text-slate-950">Le questionnaire partenaire se prépare</h1><p className="mt-3 leading-relaxed text-slate-600">Cette vue s’affiche lorsqu’aucun service de collecte n’est relié à l’environnement. En production, les invitations personnelles ouvrent directement le questionnaire sécurisé.</p></div></div><dl className="mt-6 grid gap-px border border-[#515792]/15 bg-[#515792]/15 sm:grid-cols-2"><div className="bg-white px-4 py-3"><dt className="font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-[#515792]">Accès</dt><dd className="mt-1 text-sm font-semibold text-slate-800">Invitation personnelle</dd></div><div className="bg-white px-4 py-3"><dt className="font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-[#3aab8a]">Données</dt><dd className="mt-1 text-sm font-semibold text-slate-800">Collecte sécurisée</dd></div></dl><Button className="mt-6" style={{ backgroundColor: "#E07428", color: "#fff" }} asChild><a href="mailto:ulrich.fischer@memoways.com?subject=Boussole Numérique Culture — Demande d’invitation questionnaire">Demander une invitation</a></Button></section>;
}
...
function TokenQuestionnaire({ token }: { token: string }) {
  ...
  if (status === "submitted") return <section className="mx-auto mt-10 max-w-2xl rounded-3xl border border-emerald-200 bg-emerald-50 p-8 text-center sm:mt-14"><Check className="mx-auto h-10 w-10 text-emerald-700" /><h1 className="mt-4 text-2xl font-bold text-slate-950">Merci pour vos idées et feedbacks</h1><p className="mt-3 leading-relaxed text-slate-600">Vos réponses sont bien enregistrées et contribueront à préparer l’atelier de co-conception. Une copie individuelle pourra être transmise lorsque le workflow e-mail du pilote sera activé.</p></section>;
